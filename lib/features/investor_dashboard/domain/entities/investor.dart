...
/// Aggregated performance metrics of the investor's portfolio.
class PortfolioPerformance extends Equatable {
  ...
  factory PortfolioPerformance.fromJson(Map<String, dynamic> json) {
    ...
    final invested =
        ...
        (data['investedAmount'] as num?)?.toDouble() ??
        (data['invested_amount'] as num?)?.toDouble() ??
        0.0;
    ...
    final roi =
        (data['totalRoi'] as num?)?.toDouble() ??
        (data['roi'] as num?)?.toDouble() ??
        (data['averageRoi'] as num?)?.toDouble() ??
        (data['avgRoi'] as num?)?.toDouble() ??
        (data['roiPercentage'] as num?)?.toDouble() ??
        0.0;
  ...
  @override
  List<Object?> get props => [totalInvested, currentValue, totalRoi];
}

/// An item in the investor's investment history.
class InvestmentHistoryItem extends Equatable {
  const InvestmentHistoryItem({
    required this.id,
    required this.startupName,
    required this.amount,
    required this.equity,
  ...
  factory InvestmentHistoryItem.fromJson(Map<String, dynamic> json) {
    ...
    return InvestmentHistoryItem(
      ...
      logoUrl:
          ...
          startupProfile?['logoUrl'] as String?,
    );
  }
